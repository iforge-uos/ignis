...
export const interact = auth
  ...
  .handler(async ({ input: { session_id, interaction_id, answers }, context: { tx, $user } }) => {
    ...
    if (next_section === null) {
      const pre_existing = e.select($user, () => ({
        training: {
          filter_single: {
            ...
            set: {
              training: {
                "+=": e.select(e.training.Training, () => ({
                  ...
                  "@in_person_created_at": e.select(pre_existing["@in_person_created_at"]),
                  "@in_person_signed_off_by": e.select(pre_existing["@in_person_signed_off_by"]),
                  "@infraction": e.select(pre_existing["@infraction"]),
                  filter_single: {
                    id: session.training.id,
                  },
                })),
