...
type getSharedParentScalar<A, B> =
  ...
  A extends _extpgvector.$vector ?
    ...
    B
    :
    B extends _extpgvector.$halfvec ?
    A
    :
    never
  :
  A extends _extpgvector.$sparsevec ?
    B extends _extpgvector.$sparsevec ?
    B
    :
    never
  :
  A extends _extpgvector.$halfvec ?
    B extends _extpgvector.$vector ?
    B
    :
    B extends _extpgvector.$halfvec ?
...
function getSharedParentScalar<A extends $.ScalarType, B extends $.ScalarType>(a: A, b: B): A | B {
  ...
  if (a.__name__ === "ext::pgvector::vector") {
    ...
    if(b.__name__ === "ext::pgvector::halfvec") {
      return a;
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`);
    }
  if (a.__name__ === "ext::pgvector::sparsevec") {
    if(b.__name__ === "ext::pgvector::sparsevec") {
      return b;
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`);
    }
  if (a.__name__ === "ext::pgvector::halfvec") {
    if(b.__name__ === "ext::pgvector::vector") {
      return b;
    }
    if(b.__name__ === "ext::pgvector::halfvec") {
...
const implicitCastMap = new Map<string, Set<string>>([
  ["ext::pgvector::halfvec", new Set(["ext::pgvector::vector"])],
