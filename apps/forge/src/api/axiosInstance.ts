...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      // @ts-ignore: I CBA figuring out how to get this to type correctly.
      throw notFound({ forbidden: true }); // TODO there may be a better way to signal this, might be better if this goes through the normal GenericError?
    } else {
      return Promise.reject(error);
    }
  },
