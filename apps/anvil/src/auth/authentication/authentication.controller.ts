...
export class AuthenticationController {
  ...
  async validateToken(
    ...
    @Query("roles") requiredRoles_?: string,
  ) {
    const requiredRoles = requiredRoles_?.split(",");
    this.logger.log(
      `Validating access token${requiredRoles ? ` for role: ${requiredRoles}` : ""}`,
      AuthenticationController.name,
    );
    ...
    try {
      if (accessToken) {
        await this.authService.validateAccessToken(accessToken, requiredRoles);
    ...
    const { access_token, refresh_token, csrf_token } = await this.refreshTokens(refreshToken, requiredRoles);
  ...
  private async refreshTokens(refreshToken: string, requiredRoles?: string[]) {
    ...
    if (requiredRoles && !user.roles.some((role) => requiredRoles.includes(role.name))) {
      this.logger.warn(`User does not have the required role: ${requiredRoles}`, AuthenticationController.name);
