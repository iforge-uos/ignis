...
const ActiveLocationSelector = () => {
    ...
    const {data: locationStatuses, isLoading, isError, error} = useQuery({
            ...
            <div
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                        ...
                        <PopoverContent className="w-[200px] p-0">
                            <Command>
                                ...
                                <CommandGroup>
                                    {(isLoading ? <div className="flex items-center justify-center h-[40px]"><PulseLoader color="#e11d48" size={15}/> </div>: <>
                                        {locationStatuses && locationStatuses!.map((location, index) => (
                                            <CommandItem
                                                key={index}
                ...
                {activeLocationStatus && !isLoading && (
                    <div className="flex items-center gap-2 mt-2 lg:mt-0">
                        ...
                        {/* Open Status */}
                        {(activeLocationStatus.open ? (<span className="text-green-500">OPEN</span>) : (
                        ...
                        <span
                            className="text-navbar-foreground"> {activeLocationStatus.count}/{activeLocationStatus.max}</span>
                        <span className="text-gray-500 dark:text-gray-400">Max Users </span>
                        {/* Queue Status */}

                        {(activeLocationStatus.needs_queue ? (<span className="text-red-500">Queue Needed</span>) : (
