...
export const promote = rep
  .route({ method: "POST", path: "/promote" })
  ...
  .handler(async ({ input: { id, team_ids, status }, context: { tx } }) => {
    const { direct_links, rep_type_id } = (await e
      .select({
        direct_links: e.select(
          ...
          (link) => ({
            filter: e.op(e.op(e.len(link.computed_fields), "=", 0), "and", e.op(link.name, "!=", "__type__")),
          }),
        ).name,
    ...
    console.log(direct_links, rep_type_id);
    // await e
    //   .insert(e.users.Rep, () => ({
    //     id,
    //     roles: e.select(e.users.Role, (r) => ({ filter: e.op(r.name, "in", e.set("Rep", "User")) })),
    //   }))
    //   .run(tx);
