...
import { ManageUserWidget } from "./ManageUserWidget";
import { SignInReasonWithToolsDisplay } from "./SignInReasonDisplay";
    ...
    <Card className={cn("bg-card w-60 md:w-75 p-4 rounded-sm flex flex-col justify-between")} ref={drag}>
      ...
      <div className="flex-grow">
        {!(reason?.name === REP_ON_SHIFT || reason?.name === REP_OFF_SHIFT) && (
          <SignInReasonWithToolsDisplay id={id} user={user} tools={tools!} reason={reason!} key={user.id} />
        )}
      </div>
