...
import type * as _auth from "./modules/auth";
export type scalarAssignableBy<T extends $.ScalarType> =
  ...
  T extends _auth.$PermissionSubject ? _auth.$PermissionSubject : 
  T extends _auth.$PermissionAction ? _auth.$PermissionAction : 
  never
...
type getSharedParentScalar<A, B> =
  ...
  A extends _cal.$date_duration ?
    ...
    never
  :
  A extends _auth.$PermissionSubject ?
    B extends _auth.$PermissionSubject ?
    B
    :
    never
  :
  A extends _auth.$PermissionAction ?
    B extends _auth.$PermissionAction ?
    B
    :
    never
  :
...
function getSharedParentScalar<A extends $.ScalarType, B extends $.ScalarType>(a: A, b: B): A | B {
  ...
  if (a.__name__ === "auth::PermissionSubject") {
    if(b.__name__ === "auth::PermissionSubject") {
      return b;
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`);
    }
  if (a.__name__ === "auth::PermissionAction") {
    if(b.__name__ === "auth::PermissionAction") {
      return b;
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`);
    }
