...
const UserSettingsPageComponent = () => {
  ...
  const [timeWarningFinished, setTimeWarningFinished] = useState<number>();
  const [isButtonDisabled, setIsButtonDisabled] = useState(false);
  const [remainingSeconds, setRemainingSeconds] = useState(0);

  useEffect(() => {
    if (!timeWarningFinished) {
    ...
    const checkInterval = setInterval(() => {
      const now = Date.now();
      const remaining = timeWarningFinished - now;
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <div
                        onMouseEnter={() => setTimeWarningFinished(Date.now() + 5_000)}
                        onMouseLeave={() => setTimeWarningFinished(undefined)}
                        className="relative"
                      >
                        <AlertDialogAction
                          onClick={handleDeleteAccount}
                          className="bg-destructive text-destructive-foreground hover:bg-destructive/90 relative overflow-hidden transition-all duration-200 min-w-[140px]"
                        ...
                        >
                          ...
                          <span className="relative z-10">
                            {isButtonDisabled ? `Wait ${remainingSeconds}s...` : "Delete Account"}
                          </span>
                        </AlertDialogAction>
                      </div>
                    </AlertDialogFooter>
