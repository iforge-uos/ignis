import { LocationName, QueueEntry } from "@packages/types/sign_in";
import { Container, Hr, Text } from "jsx-email";
import * as React from "react";
import { Email } from "../components/heading";
import { Link } from "../components/link";

export function Template({ id = "0000-0000-0000-0000", location = "{location}" }: QueueEntry & { location: string }) {
  return (
    <Email
      preview="You're in the queue for the iForge"
      title="iForge queue request received"
      heading={`You're in the queue for the iForge ${location}`}
    >
      <Hr />
      <Container>
        <Text>
          Hey there!
          <br />
          You've been added to the queue at {new Date(new Date().getTime()).toLocaleTimeString()}. You'll get an email
          when a place is available.
        </Text>

        <Text className="text-center text-gray-500 text-xs!">
          If you no longer require your place please{" "}
          <Link
            href={`https://iforge.sheffield.ac.uk/${location}/queue/delete/${id}`} // FIXME this doesn't exist yet
          >
            click here
          </Link>{" "}
          to cancel.
        </Text>
      </Container>
    </Email>
  );
}

export default Template;
