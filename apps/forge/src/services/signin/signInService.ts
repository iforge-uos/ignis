...
export const PostSignIn = async ({locationName, uCardNumber, signal, postBody}: PostSignInProps): Promise<string> => {
    try {
        const {data} = await axiosInstance.post(`/location/${locationName}/sign-in/${uCardNumber}`, postBody, {signal: signal});
        return data;
    } catch (error) {
...
export const PostSignOut = async ({locationName, uCardNumber, signal}: PostSignOutProps): Promise<string> => {
    try {
        const {data} = await axiosInstance.post(`/location/${locationName}/sign-out/${uCardNumber}`,{} ,{signal: signal});
        return data;
    } catch (error) {
...
export const PostRegister = async ({
                                       ...
                                       username,
                                       signal
                                   }: PostRegisterProps): Promise<string> => {
    try {
        const {data} = await axiosInstance.post(`/location/${locationName}/register-user`, {
            username: username,
